...
#define GOS_IOCTL_DEBUG_GET_ARG   5  // devuelve un valor de 32 bits seteado por el kernel antes de crear el proceso (solo para pruebas de aislamiento)
...
// Solo para pruebas de aislamiento entre procesos: devuelve un valor de
// 32 bits que el kernel guardo ANTES de crear este proceso (ver
// debug_set_test_arg en process.c). No es parte de la ABI "real" — es un
// atajo deliberado para poder testear ring3-a-ring3 sin IPC de verdad.
static inline uint32_t gos_debug_get_arg(void) {
    return gos_syscall(GOS_SYS_IOCTL, GOS_IOCTL_DEBUG_GET_ARG, 0, 0, 0);
}

static inline void gos_print_hex(uint32_t val) {
    char out[11];
    out[0] = '0'; out[1] = 'x';
    for (int i = 0; i < 8; i++) {
        uint8_t nibble = (uint8_t)((val >> ((7 - i) * 4)) & 0xF);
        out[2 + i] = (char)(nibble < 10 ? ('0' + nibble) : ('a' + nibble - 10));
    }
    out[10] = 0;
    gos_write(1, out, 10);
    gos_write(1, "\n", 1);
