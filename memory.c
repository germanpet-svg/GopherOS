...
// memory_get_stats() - Usado por statusbar.c para el pie de pagina.
void memory_get_stats(uint32_t* used_pages, uint32_t* total_pages) {
    uint32_t used = 0;
    for (size_t i = 0; i < ARENA_PAGES; i++) if (page_bitmap[i]) used++;
    if (used_pages) *used_pages = used;
    if (total_pages) *total_pages = ARENA_PAGES;
}
...
// page_alloc_mapped() - Como page_alloc(), pero para consumidores que
// necesitan escribir contenido Y conocer la direccion "fisica" (=virtual
// en este diseño identity-map) que hay que pasarle a paging_map_page()/
// paging_grant_access(). Usado por el loader de ELF (process.c) para traer
// cada segmento PT_LOAD a paginas propias, no compartidas.
void* page_alloc_mapped(size_t num_pages, phys_addr_t* out_phys) {
    phys_addr_t raw = page_alloc(num_pages);
    if (raw == 0) { if (out_phys) *out_phys = 0; return NULL; }
    void* v = phys_to_virt(raw);
    if (out_phys) *out_phys = (phys_addr_t)(uintptr_t)v;
    return v;
}
...
// Accesores usados por proc_create_ring3() para saber qué rango físico
// marcar como accesible-desde-ring3 vía paging_set_user_access().
phys_addr_t region_backing_phys(Region* r) { return r ? (phys_addr_t)(uintptr_t)phys_to_virt(r->backing_phys) : 0; }
size_t region_backing_size(Region* r) { return r ? (r->backing_pages * PAGE_SIZE) : 0; }
